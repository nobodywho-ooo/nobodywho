...
pub fn init_logging() {
    ...
    //
    // `tracing` events, llama.cpp's own lines included, reach it through
    // `nobodywho::logging::forward_tracing_to_log` below.
    //
...
impl SamplerBuilder {
    ...
    pub fn dynamic_temperature(
        ...
        Arc::new(SamplerBuilder {
            inner: self
                .inner
                .clone()
                .shift(nobodywho::sampler::ShiftStep::DynamicTemperature {
                    temperature,
                    delta,
                    exponent,
                }),
    ...
    pub fn top_n_sigma(&self, n: f32) -> Arc<SamplerBuilder> {
        Arc::new(SamplerBuilder {
            inner: self
                .inner
                .clone()
                .shift(nobodywho::sampler::ShiftStep::TopNSigma { n }),
        })
    }
    ...
    pub fn logit_bias(&self, biases: HashMap<i32, f32>) -> Arc<SamplerBuilder> {
        Arc::new(SamplerBuilder {
            inner: self
                .inner
                .clone()
                .shift(nobodywho::sampler::ShiftStep::LogitBias { biases }),
        })
    }
