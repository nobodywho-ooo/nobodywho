...
Future<String> downloadLibrary(Config config, String version) async {
  ...
  try {
    ...
    // Download the dlopen'd ggml/llama sibling libs listed in the release's per-triple
    // manifest (generated by `ls` at release, so it always matches the CPU-variant set
    // that build produced — no hardcoded lib names here). Each is saved next to the
    // binding, where the desktop CMake glob / Android jniLibs copy find it and the
    // binding's rpath / our load_backends_from_path resolve it. (Apple platforms take the
    // xcframework branch, never this path.)
    final relBase =
        'https://github.com/nobodywho-ooo/nobodywho/releases/download/nobodywho-flutter-v$version';
    ...
    final manifest = await fetch('$relBase/nobodywho-siblings-$triple.txt');
    if (manifest == null) {
      throw Exception('Failed to download sibling manifest for $triple');
    }
    for (final name in String.fromCharCodes(manifest)
        .split('\n')
        .map((l) => l.trim())
        .where((l) => l.isNotEmpty)) {
      final destFile = File('$cacheDir/$name');
      if (destFile.existsSync()) continue;
      final dot = name.lastIndexOf('.');
      final assetName =
          '${name.substring(0, dot)}-$triple-${config.buildType}${name.substring(dot)}';
      final bytes = await fetch('$relBase/$assetName');
      if (bytes == null) {
        throw Exception('Failed to download sibling lib $name (manifest asset $assetName)');
      }
      await destFile.writeAsBytes(bytes);
      stderr.writeln('Downloaded sibling lib: $name');
    }
