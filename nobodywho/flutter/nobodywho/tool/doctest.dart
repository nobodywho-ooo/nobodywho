...
String normalizeImport(String importLine) {
  ...
  importLine = importLine.replaceAll(
    'nobodywho_dart/nobodywho_dart.dart',
    'nobodywho/nobodywho.dart',
  );
  importLine = importLine.replaceAll(
    "'package:nobodywho_dart/",
    "'package:nobodywho/",
  );
  importLine = importLine.replaceAll(
    '"package:nobodywho_dart/',
...
String removeInitCalls(String code) {
  final initRegex = RegExp(
    r'^\s*await\s+nobodywho\.NobodyWho\.init\(\)\s*;\s*\n?',
    multiLine: true,
  );
...
String generateTestFile(List<CodeGroup> groups, String testName) {
  ...
  buffer.writeln("@Timeout(Duration(seconds: 780))"); // 13 mins
  ...
  for (final group in groups) {
    ...
    final needsVisionModel =
        code.contains('vision-model.gguf') || code.contains('mmproj.gguf');
    ...
    if (hasMainFunction(code)) {
      ...
      // Indent the code
      final indentedCode = codeWithoutImports
          .split('\n')
          .map((l) => '      $l')
          .join('\n');
  ...
  for (final group in groups) {
    ...
    if (hasMainFunction(code)) {
      buffer.writeln();
      buffer.writeln(
        "// Extracted from ${group.sourceFile}:${group.startLine}",
      );
      ...
      codeWithoutImports = codeWithoutImports
          .replaceFirst(
            RegExp(r'Future<void>\s+main\s*\('),
            'Future<void> _doctest_$testIndex(',
          )
          .replaceFirst(
            RegExp(r'void\s+main\s*\('),
            'Future<void> _doctest_$testIndex(',
          );
