...
**On Linux**:
```rust
LD_LIBRARY_PATH=../../target/debug/ flutter test test/nobodywho_test.dart
```
**On Mac**:
...
flutter test test/nobodywho_test.dart
```
...
**On Linux**
```rust
dart run tool/doctest.dart ../../../docs/docs-flutter --generate-only
...
Android does not use this resolver. Gradle resolves the multi-ABI
`ai.nobodywho:nobodywho-flutter-android` AAR whose version matches this
Flutter package from Maven Central.
