...
impl ChatHandle {
    ...
    pub async fn say_complete(&self, text: impl Into<String>) -> Result<String, SayError> {
        self.say_complete_with_config(text, SamplerConfig::default(), vec![]).await
    }
...
impl ToolBuilder {
    ...
    pub fn param_with_schema<S: Into<String>>(mut self, name: S, schema: serde_json::Value) -> Self {
    ...
    pub fn build(self) -> Tool {
        ...
        Tool {
            ...
            json_schema,
            function: self.handler.unwrap_or_else(|| {
                Arc::new(|_| "Tool handler not implemented".to_string())
            }),
        }
    }
}
