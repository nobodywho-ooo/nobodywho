...
mod tests {
    ...
    #[test]
    fn test_qwen3_template_empty_messages_should_not_fail() {
        // Verifies that rendering with an empty messages array does not cause an error.
        // Some chat templates (like Qwen3) directly access messages[0] without bounds checking.
        // The render function should handle this gracefully by injecting a placeholder message.
        ...
        let messages: Vec<Message> = vec![];
        let result = chat_template.render(&messages, &ctx);

        assert!(
            result.is_ok(),
            "Rendering empty messages should not fail, but got error: {:?}",
            result.err()
        );
    }
}
