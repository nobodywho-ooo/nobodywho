...
impl SupertonicBackend {
    fn call(&mut self, text: &str) -> Result<Vec<f32>, TtsError> {
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            let (chunk_wav, duration) = self.infer(std::slice::from_ref(chunk))?;
            append_chunk_audio(
                &mut wav,
                &chunk_wav,
                duration[0],
    ...
    fn infer(&mut self, text_list: &[String]) -> Result<(Vec<f32>, Vec<f32>), TtsError> {
        ...
        let encoded_texts = self
            .text_processor
            .encode_texts(text_list, &self.language)?;
        let duration = self.predict_duration(&encoded_texts)?;
        let style_ttl_tensor = self.style.ttl_tensor()?;
    ...
    fn denoise_latent(
        ...
        let total_step_tensor =
            Tensor::from_array(Array::from_elem(batch_size, self.settings.steps as f32))?;
