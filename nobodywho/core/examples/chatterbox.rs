...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: {} <model-dir> \"text\" [language] [voice.wav]",
            args[0]
        );
        std::process::exit(1);
    }

    let mut cfg = ChatterboxConfig::new(&args[1]);
    if let Some(l) = args.get(3) {
        cfg.language = l.clone();
    }
    if let Some(wav) = args.get(4) {
        cfg.reference_wav = Some(wav.into());
    }

    let tts = TtsBuilder::new(TtsConfig::Chatterbox(cfg)).build()?;
