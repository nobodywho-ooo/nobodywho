...
// ---------- EncoderSync / CrossEncoderSync (mirrors Python's sync variants) ----------
//
// `EncoderSync` / `CrossEncoderSync` block the calling thread until the work
// completes, then return the result directly (no Promise wrapping). Useful in
// contexts where blocking is fine — Web Worker scripts, Node CLIs, any caller
// that would otherwise `await` the async variant and immediately need the
// result anyway.
//
// Implementation: thin wrappers around `nobodywho::encoder::Encoder` /
// `nobodywho::crossencoder::CrossEncoder`, both of which drive their async
// counterpart via `futures::executor::block_on` internally. On native that's
// a real `Mutex+Condvar` park; on wasm32 (single-threaded) it polls the
// shared task queue until the worker future completes. Wall-clock cost is
// identical to the async variants — only the JS-side API shape changes.

/// Synchronous variant of [`Encoder`]. Calls block the JS thread until the
/// embedding is ready and return a `Float32Array` directly.
#[wasm_bindgen]
pub struct EncoderSync {
    inner: nobodywho::encoder::Encoder,
}

#[wasm_bindgen]
impl EncoderSync {
    /// Create a new sync encoder. `n_ctx` defaults to 4096 if omitted.
    #[wasm_bindgen(constructor)]
    pub fn new(model: &Model, n_ctx: Option<u32>) -> EncoderSync {
        EncoderSync {
            inner: nobodywho::encoder::Encoder::new(
                Arc::clone(&model.inner),
                n_ctx.unwrap_or(4096),
            ),
        }
    ...
    /// Generate an embedding vector for the given text. Blocks until ready
    /// and returns a `Float32Array`.
    pub fn encode(&self, text: String) -> Result<js_sys::Float32Array, JsError> {
        let embedding = self
            .inner
            .encode(text)
            .map_err(|e| JsError::new(&e.to_string()))?;
...
/// Synchronous variant of [`CrossEncoder`]. Calls block the JS thread until
/// scoring is ready and return arrays directly.
#[wasm_bindgen]
pub struct CrossEncoderSync {
    inner: nobodywho::crossencoder::CrossEncoder,
}

#[wasm_bindgen]
impl CrossEncoderSync {
    /// Create a new sync cross-encoder. `n_ctx` defaults to 4096 if omitted.
    #[wasm_bindgen(constructor)]
    pub fn new(model: &Model, n_ctx: Option<u32>) -> CrossEncoderSync {
        CrossEncoderSync {
            inner: nobodywho::crossencoder::CrossEncoder::new(
                Arc::clone(&model.inner),
                n_ctx.unwrap_or(4096),
            ),
        }
    ...
    /// Score each document against the query. Returns a `Float32Array` in
    /// the same order as the input documents.
    pub fn rank(
        &self,
        query: String,
        ...
        let scores = self
            ...
            .rank(query, documents)
            .map_err(|e| JsError::new(&e.to_string()))?;
        Ok(js_sys::Float32Array::from(scores.as_slice()))
    }

    /// Score each document and return an array of `[document, score]` pairs
    /// sorted by descending score. Mirrors Python's
    /// `CrossEncoder.rank_and_sort(...)` -> `list[tuple[str, float]]`.
    #[wasm_bindgen(js_name = rankAndSort)]
...
// ---------- ChatSync / TokenStreamSync ----------
//
// Mirrors Python's sync `Chat` + `TokenStream` alongside the existing async
// `Chat` (Promise-returning) and the worker-backed `WorkerChat`. Calls block
// the JS thread until inference completes; the returned `TokenStreamSync`
// has all tokens already buffered in a `VecDeque`-equivalent — `nextToken`
// pops one without awaiting, `completed` returns the full text.
//
// Backed by `nobodywho::chat::ChatHandleSync` (wasm32-only in core), which
// owns the `Worker` inline and calls `worker.ask(prompt, callback)` on the
// calling thread. No `spawn_local` task, no Promise wrapping, no cooperative
// scheduling.
//
// Trade-off vs in-process `Chat`: identical wall-clock cost (inference runs
// synchronously either way on wasm), but no Promise indirection at the
// call site. Useful from Web Worker scripts and Node CLIs.

/// Synchronous variant of [`Chat`]. `ask` blocks until inference completes
/// and returns a [`TokenStreamSync`] with all tokens buffered.
#[wasm_bindgen]
pub struct ChatSync {
    handle: nobodywho::chat::ChatHandleSync,
}

#[wasm_bindgen]
impl ChatSync {
    /// Create a new sync chat session bound to a loaded model.
    /// Accepts the same options object as the async [`Chat`] constructor
    /// (`systemPrompt`, `contextSize`, `templateVariables`, `constraint`).
    #[wasm_bindgen(constructor)]
    pub fn new(model: &Model, options: JsValue) -> Result<ChatSync, JsError> {
        let opts: ChatOptions = if options.is_undefined() || options.is_null() {
            ChatOptions::default()
        } else {
            serde_wasm_bindgen::from_value(options).map_err(|e| JsError::new(&e.to_string()))?
    ...
    /// [`TokenStreamSync`] whose tokens are already buffered.
    pub fn ask(&mut self, prompt: String) -> TokenStreamSync {
        let stream = self.handle.ask(prompt);
        TokenStreamSync { inner: stream }
    }

    /// Reset the conversation. Optionally provide a new system prompt.
    /// Tools aren't yet exposed in the wasm binding; passes an empty list.
    pub fn reset(&mut self, system_prompt: Option<String>) -> Result<(), JsError> {
        self.handle
            .reset_chat(system_prompt, Vec::new())
            .map_err(|e| JsError::new(&e.to_string()))
    }

    /// Clear the chat history, keeping the system prompt and tools.
    #[wasm_bindgen(js_name = resetHistory)]
    pub fn reset_history(&mut self) -> Result<(), JsError> {
        self.handle
            .reset_history()
            .map_err(|e| JsError::new(&e.to_string()))
    }
...
/// Synchronous variant of [`TokenStream`]. All tokens were generated by
/// the time this stream was returned, so `nextToken` never blocks beyond
/// a queue pop.
#[wasm_bindgen]
pub struct TokenStreamSync {
    inner: nobodywho::chat::TokenStream,
}

#[wasm_bindgen]
impl TokenStreamSync {
    /// Pop the next token. Returns `undefined` when the stream is drained.
    #[wasm_bindgen(js_name = nextToken)]
    ...
    pub fn completed(&mut self) -> Result<String, JsError> {
        self.inner
            .completed()
            .map_err(|e| JsError::new(&e.to_string()))
    }
