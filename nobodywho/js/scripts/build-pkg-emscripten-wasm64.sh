#!/usr/bin/env bash
# Build nobodywho-js's deployable artifacts under wasm64-unknown-emscripten
# (MEMORY64 — a 16 GiB heap, for models whose working set exceeds wasm32's 4 GiB).
#
# This is the wasm64 sibling of build-pkg-emscripten.sh. The two are
# DELIBERATELY near-identical: every step below — marker injection, the
# wasm-bindgen sed patches, the addToLibrary strip, and the defer-_initialize
# post-link patch — is shared verbatim with the wasm32 script and MUST be kept
# in sync (treat build-pkg-emscripten.sh as the source of truth). Only these
# differ for wasm64:
#   - cargo builds the custom wasm64 target spec (js/targets/…json) rather than
#     the built-in wasm32 triple (-Zbuild-std compiles std for the tier-3 target);
#   - the emcc --post-link adds -sMEMORY64=1 -sWASM_BIGINT and a 16 GiB ceiling;
#   - outputs land in pkg-bundler-wasm64/ so the wasm32 + wasm64 builds coexist.
#
# Produces nobodywho/js/pkg-bundler-wasm64/ with:
...
#   - A recent nightly Rust (>= 2026-06-08) with the rust-src component.
#     wasm64 std is built from source via -Zbuild-std, which compiles
#     library/unwind — and that only gained the wasm64
#     `unwinder_private_data_size` const in rust-lang/rust#156573 (merged
#     2026-06-07). Older nightlies fail the std build with a cryptic E0425.
...
TARGET_DIR="$ROOT/nobodywho/target/wasm64-unknown-emscripten/release"
...
if [[ ! -f "$TARGET_SPEC" ]]; then
  echo "error: wasm64 target spec not found at $TARGET_SPEC" >&2
  exit 1
fi

# Verify the nightly carries rust-lang/rust#156573 (wasm64 unwinder support).
# Without it, -Zbuild-std fails deep inside library/unwind with a cryptic
# E0425 on `unwinder_private_data_size`; detect it up front so the failure
# is legible.
RUST_SRC="$(rustup run nightly rustc --print sysroot 2>/dev/null)/lib/rustlib/src/rust"
LIBUNWIND="$RUST_SRC/library/unwind/src/libunwind.rs"
if [[ ! -f "$LIBUNWIND" ]]; then
  echo "error: rust-src not found for nightly ($LIBUNWIND)." >&2
  echo "       -Zbuild-std needs it: rustup component add rust-src --toolchain nightly" >&2
  exit 1
fi
if ! grep -Eq 'target_family = "wasm".*target_os = "emscripten"' "$LIBUNWIND"; then
  echo "error: this nightly's library/unwind lacks wasm64 unwinder support" >&2
  echo "       (rust-lang/rust#156573, merged 2026-06-07). Update: rustup update nightly" >&2
  exit 1
fi
...
# deliberately do NOT pre-warm with `embuilder build SYSTEM --wasm64`: SYSTEM
# builds every variant (ASan, debug, single- + multi-threaded, …), almost all
# of which we never link — many minutes of wasted work. The lazy build during
# the cargo step is faster and sufficient (first wasm64 build pays it once).

# `pre.js` is checked in under scripts/ (the pkg-bundler-wasm64/ dir is
...
echo "==> cargo build --target $TARGET_SPEC --release -p nobodywho-js (-Zbuild-std)"
rm -rf "$TARGET_DIR"
(
  cd "$ROOT/nobodywho"
  # Set RUSTFLAGS explicitly instead of relying on .cargo/config.toml's
  # [target.wasm64-unknown-emscripten] rustflags. A RUSTFLAGS env var
  ...
  cargo +nightly build --release --target "$TARGET_SPEC" -p nobodywho-js \
...
echo
echo "Test:"
echo "  PATH=/opt/homebrew/bin:\$PATH node $JS_DIR/tests/test_emscripten_wasm64.mjs <embedding.gguf>"
