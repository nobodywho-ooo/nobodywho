...
fn compatible_completion_options(
    temperature: Option<f32>,
    top_p: Option<f32>,
    seed: Option<u32>,
) -> PyResult<nobodywho::chat::Options> {
    let sampler =
        nobodywho::sampler::SamplerConfig::from_openai_parameters(temperature, top_p, seed)
            .map_err(|error| pyo3::exceptions::PyValueError::new_err(error.to_string()))?;
    Ok(nobodywho::chat::Options::new().with_sampler(sampler))
}
...
impl<'py> ModelOrPath<'py> {
    fn source(&self) -> PyResult<String> {
        match self {
            Self::ModelObj(model) => Ok(model.borrow().model.source().to_string()),
...
struct NobodyWhoState {
    cached_model: Mutex<Option<Arc<nobodywho::llm::Model>>>,
}
...
impl NobodyWhoState {
    fn model_for(
        ...
        let requested_source = requested.source()?;
        ...
        let model = py
            .detach(|| -> Result<Arc<nobodywho::llm::Model>, String> {
                ...
                if let Some(model) = cached
                    .as_ref()
                    .filter(|cached| cached.source() == requested_source)
                    .map(Arc::clone)
                {
                    return Ok(model);
                }
                let model = nobodywho::llm::get_model(&requested_source, true, None, None, None)
                    .map(Arc::new)
...
fn request_completion_stream(
    ...
    let completion_options = compatible_completion_options(temperature, top_p, seed)?;
    ...
    fn next_chunk(&mut self, py: Python<'_>) -> PyResult<Option<ChatCompletionChunk>> {
        ...
        loop {
            ...
            if let Some(response) = self.pending_response.take() {
                let completion = compatible_completion_result(
                    response,
                    self.id.clone(),
                    self.created,
                    self.model.clone(),
            ...
            match chunk {
                Some(nobodywho::chat::CompletionChunk::Token(token)) => {
                    self.pending_deltas.extend(self.parser.push(&token));
                }
                Some(nobodywho::chat::CompletionChunk::Done(response)) => {
...
impl ChatCompletionsResource {
    ...
    #[pyo3(signature = (*, model: "Model | os.PathLike | str", messages: "list[dict]", temperature: "float | None" = None, top_p: "float | None" = None, seed: "int | None" = None, template_variables: "dict[str, bool] | None" = None, tools: "list[Tool] | None" = None, thinking: "bool | None" = None, max_tokens: "int | None" = None, max_completion_tokens: "int | None" = None, stream = false, n_ctx = 4096, n_threads: "int | None" = None, mtp: "MtpConfig | None" = None) -> "ChatCompletion | ChatCompletionStream")]
    fn create(
        &self,
        model: ModelOrPath<'_>,
...
impl ResponsesResource {
    ...
    #[pyo3(signature = (*, model: "Model | os.PathLike | str", input: "str | list[dict]", instructions: "str | None" = None, temperature: "float | None" = None, top_p: "float | None" = None, seed: "int | None" = None, template_variables: "dict[str, bool] | None" = None, tools: "list[Tool] | None" = None, thinking: "bool | None" = None, max_output_tokens: "int | None" = None, stream = false, n_ctx = 4096, n_threads: "int | None" = None, mtp: "MtpConfig | None" = None) -> "Response | ResponseStream")]
    fn create(
        &self,
        model: ModelOrPath<'_>,
        input: &Bound<'_, PyAny>,
...
fn compatible_completion_result(
