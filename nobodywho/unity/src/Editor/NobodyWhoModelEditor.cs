...
namespace NobodyWho {
    ...
    public class ModelEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("useGpuIfAvailable"), 
                new GUIContent("Use GPU If Available", "Whether to use GPU acceleration if available"));
    ...
    public class ChatEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUILayout.PropertyField(modelProp, new GUIContent("Model", "The model node for the chat"));
            if (modelProp.objectReferenceValue == null || !(modelProp.objectReferenceValue is Model)) {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("systemPrompt"), 
                new GUIContent("System Prompt", "The system prompt for the chat, this is the basic instructions for the LLM's behavior"));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("stopWords"), 
                new GUIContent("Stop Words", "Stop words to stop generation at these specified tokens, seperated by commas"));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("contextLength"), 
                new GUIContent("Context Length", "Maximum number of tokens that can be stored in the chat history. Higher values use more VRAM, but allow for longer 'short term memory' for the LLM"));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("use_grammar"), 
                new GUIContent("Use Grammar", "Enable grammar-based output formatting"));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("grammar"), 
                new GUIContent("Grammar", "Grammar rules to structure the model's output"));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("onToken"), 
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("onComplete"), 
                new GUIContent("On Complete", "Triggered when the LLM has finished generating the response"));
    ...
    public class EmbeddingEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUILayout.PropertyField(modelProp, 
                new GUIContent("Model", "The model node for the embedding. Required for comparing text similarity without exact matching."));
            if (modelProp.objectReferenceValue == null || !(modelProp.objectReferenceValue is Model)) {
                EditorGUILayout.HelpBox("A Model component is required.", MessageType.Error);
            }

            EditorGUILayout.Space(10);
            EditorGUILayout.PropertyField(serializedObject.FindProperty("onEmbeddingComplete"), 
                new GUIContent("On Embedding Complete", "Triggered when the embedding has finished generating. Returns the embedding as a float array."));
            
            EditorGUILayout.PropertyField(serializedObject.FindProperty("onError"), 
                new GUIContent("On Error", "Triggered when an error occurs during embedding generation."));
