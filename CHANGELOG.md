...
- A response's finished text, and the assistant message stored in the chat history, is now rebuilt from what the model wrote instead of copied from the token stream. Reasoning always has its tags, even when the chat template opened it, while the formatting before a tool call and stray end markers are left out. So the history renders the way the model wrote it. The formatting before a tool call is no longer streamed either. Affects all bindings.
