...
- Handing `complete()` both a sampler and a set of tools no longer recompiles the tool-calling grammar twice for that turn, and changing only the sampler no longer recompiles it at all. The llguidance tokenizer state a grammar is compiled against depends on the model rather than the grammar, so it is now built once per chat and reused, turning hundreds of milliseconds of per-turn overhead into single-digit milliseconds. Available for all bindings.
