...
- **Breaking:** `SamplerConfig` holds its constraints in a new `constraining_steps` list instead of mixing them into `steps`, and `json_schema`, `regex` and `lark` are no longer sampler steps. The chain runs the constraints, then the steps, then the sample step, so a constraint cannot end up behind a truncation step that leaves it nothing valid to pick. Shift steps run in the order you add them, which matters for `dry`, `penalties` and `logit_bias`: they only reshuffle a shortlist if you chain them after a truncation step. Note that llama.cpp's own default chain leads with the penalties. `SamplerConfig.from_json()` rejects a saved config with a constraint inside `steps`, naming the step it could not read; move that entry into a `"constraining_steps"` list to load it. Affects all bindings; the `from_json()` rejection applies to the five that expose it — Python, Flutter, Kotlin, Swift and React Native.
