...
- `SamplerBuilder` gained `constrain_with_json_schema`, `constrain_with_regex`, `constrain_with_grammar` and `json`, so a constraint can be combined with a temperature or a repetition penalty — the equivalent `SamplerPresets` each produce a finished sampler and cannot be layered. Constraining steps and the steps that reweight the whole vocabulary — `dry`, `penalties` and `logit_bias` — always run before the other shift steps, in the order you chain them, since a constraint applied after a truncation step can find none of the surviving tokens valid and abort generation, and a penalty or bias applied after one only reshuffles a shortlist picked without it. Available for all bindings.
