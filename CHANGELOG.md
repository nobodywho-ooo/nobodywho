...
- **Breaking:** the system prompt is no longer stored as the first chat message; it is a setting on the `Chat`, as in the Anthropic and Gemini SDKs. `get_chat_history()` therefore does not return the leading system message, and `complete()` no longer clears the system prompt when the list you pass has none. A system message further into the list stays in the history instead, for the chat template to render in place — on a model without a system role, where the system prompt is folded into the first user message and only a leading one can be, generating reports an error saying where to move the instruction. Media in a system message is now rejected, since no chat template supports it. Affects all bindings.
