...
- `SamplerBuilder` gained `constrain_with_json_schema`, `constrain_with_regex`, `constrain_with_grammar` and `json`, so a constraint can be combined with a temperature or a repetition penalty — the equivalent `SamplerPresets` each produce a finished sampler and cannot be layered. Constraining steps always run before the other shift steps regardless of where you chain them, since a constraint applied after a truncation step can find none of the surviving tokens valid and abort generation. Available for all bindings.
...
- **Breaking:** the system prompt is no longer stored as the first chat message; it is a setting on the `Chat`, as in the Anthropic and Gemini SDKs. `get_chat_history()` therefore does not return the leading system message, and `complete()` no longer clears the system prompt when the list you pass has none. A system message further into the list stays in the history instead, for the chat template to render in place — templates without a system role report an error for those, since only a leading one can be folded into the first user message. Media in a system message is now rejected, since no chat template supports it. Affects all bindings.
