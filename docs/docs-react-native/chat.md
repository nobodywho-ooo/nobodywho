...
```typescript
await chat
  .complete([{ role: "user", content: "Name one fruit." }], {
...
await chat.complete([{ role: "user", content: "Name another." }]).completed();
```
...
const chat = await Chat.fromPath({
  ...
  contextShift: {
    keepFirstTurns: 1,
    keepLastTurns: 4,
    target: ShiftTarget.Fraction.new({ fraction: 0.75 }),
  },
});
```

To turn context shifting off, pass `contextShift: { enabled: false }`; a full context then throws instead. The options can also be changed on an existing chat:

```typescript continuation
await chat.setContextShift({ enabled: false });
```
...
Load the model with a compatible draft-heads gguf (e.g. `mtp-gemma-4-E2B-it.gguf` for Gemma-4-E2B) and pass an `mtp` config when constructing the chat:
...
// `{}` uses the default drafter tuning; override fields with e.g. `{ kMax: 5 }`
const chat = new Chat({ model, mtp: {} });
```

`Chat.fromPath` accepts the same two options if you don't need to share the model:
