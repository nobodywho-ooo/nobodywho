...
```typescript
const sampler = new SamplerBuilder().constrainWithRegex("yes|no").temperature(0.8).dist() as SamplerConfig;
const same = new SamplerBuilder().temperature(0.8).constrainWithRegex("yes|no").dist() as SamplerConfig;
```
