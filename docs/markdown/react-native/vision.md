...
Use `Prompt` to build prompts that mix text and images, then pass them to `chat.ask()`:
...
const response = await chat
  .ask(
    new Prompt([
      Prompt.Text("Tell me what you see in the images."),
      Prompt.Image("/path/to/dog.png"),
      Prompt.Image("/path/to/penguin.png"),
    ]),
  )
  .completed();
```
