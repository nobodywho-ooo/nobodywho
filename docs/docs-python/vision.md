...
`get_chat_history()` records the file path of every image and audio clip alongside the message
it belongs to, so a conversation containing media can be saved and replayed later:
