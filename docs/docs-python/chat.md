...
A system message further into the list stays in the history, for the chat template to render in place. Templates without a system role — Gemma 2, Mistral v0.3 — raise an error instead, since only a leading one can be folded into the first user message.
