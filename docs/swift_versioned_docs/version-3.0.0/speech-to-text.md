...
```swift
let stt = try await SpeechToText.load(source: "hf://onnx-community/whisper-base", quantization: "q4")
```
...
```swift
let stt = try await SpeechToText.load(source: "hf://onnx-community/whisper-base", language: "en")
```
